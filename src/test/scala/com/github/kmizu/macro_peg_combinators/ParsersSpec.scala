...
import com.github.kmizu.macro_peg_combinators.Parsers._
import org.scalatest.prop.GeneratorDrivenPropertyChecks
import org.scalacheck.{Arbitrary, Gen}

class ParsersSpec extends FunSpec with DiagrammedAssertions with GeneratorDrivenPropertyChecks {
  describe("Parsers Example") {
    it("palindrome") {
      object Palindrome {
        lazy val S: Parser[Any] = P("".s) ~ !any
        def P(r: Parser[Any]): Parser[Any] = "a".s ~ refer(P("a".s ~ r)) / "b".s ~ refer(P("b".s ~ r)) / r
      }
      val S = Palindrome.S
